...
for(var j=0;j<jitterFrames;j++) {
	...
	for(var i=0;i<verticeCount;i++) {
		...
		if i > 0 {
			...
			if i == verticeCount-1 and point_distance(vertex.x,vertex.y, x,y) > 5 {
				vertex.x += lengthdir_x(1, point_direction(vertex.x,vertex.y, x,y))
				vertex.y += lengthdir_y(1, point_direction(vertex.x,vertex.y, x,y))
			}
